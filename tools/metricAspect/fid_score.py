...
#adapted from  https://github.com/layer6ai-labs/dgm-eval
def compute_statistics_fd(reps):
    ...
    return mu, sigma
#adapted from  https://github.com/layer6ai-labs/dgm-eval
def get_representations_fd(model, dataloader, device, normalized=False):
...
def extract_features(dataloader, model):
    ...
    for imgs in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            feat = feat.view(feat.size(0), -1)  # Shape: (B, D)
            feat = feat.cpu().numpy()

        if mean is None:
            D = feat.shape[1]
            mean = np.zeros(D, dtype=np.float32)
            sum_outer = np.zeros((D, D), dtype=np.float32)

        n = feat.shape[0]
        n_samples += n

        mean += feat.sum(axis=0)
        sum_outer += feat.T @ feat  # (D, B) @ (B, D) → (D, D)

    mean /= n_samples
    cov = sum_outer / n_samples - np.outer(mean, mean)

    return mean, cov

# 3. Fréchet Distance calculation
def calculate_frechet_distance_fd(mu1, sigma1, mu2, sigma2, eps=1e-6):
