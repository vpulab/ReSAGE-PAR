...
class SeqPAR2(Blip2Base):
    def __init__(self, 
        ...
        # Comprobamos si la capa problemática está y si el tamaño choca
        if 'llama_proj.weight' in state_dict and state_dict['llama_proj.weight'].shape != self.llama_proj.weight.shape:
            print("⚠️ Aviso: Eliminando 'llama_proj' del checkpoint por diferencia de tamaño (13B vs 7B). Inicializando desde cero...")
            del state_dict['llama_proj.weight']
            del state_dict['llama_proj.bias']
            
        # Ahora sí, cargamos el resto del modelo sin que crashee
        ...
        print(f"DEBUG: len(cbam_class)={len(self.cbam_class)}")
        print(f"DEBUG: attr_group_num={self.attr_group_num}")
        print(f"DEBUG: group_details={self.group_details}")
    ...
    def infer(self, im, imgname=None, gt_label=None, max_new_tokens=60, num_beams=1, min_length=10, top_p=0.9,
        ...
        outputs = self.llama_model.generate(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
            max_new_tokens=max_new_tokens,    # <-- CORREGIDO: Usa tu variable (60), no 100
            stopping_criteria=stopping_criteria,
            num_beams=1,                      # <-- Aseguramos Greedy search puro
            do_sample=False,                  # <-- CORREGIDO: Apagamos el motor de probabilidad
            repetition_penalty=repetition_penalty,
            length_penalty=length_penalty,
            use_cache=True,                   # <-- AÑADIDO: Fuerza a usar el KV Cache explícitamente
            return_dict_in_generate=True,
            output_hidden_states=True,        # Lo necesitas para tu cbam_llm
            output_attentions=False           # <-- CORREGIDO: ¡Apagado! Ahorro masivo de velocidad y VRAM
        ) 
        answers = []
        ...
        # --- CORREGIDO: Operaciones 100% en la GPU sin atascos ---
        # Cogemos los tensores que YA están en la GPU y los apilamos directamente
        hiddens_list = [hiddens[-1] for hiddens in outputs['hidden_states'][1:]]
        hidden_states = torch.stack(hiddens_list) 
        
        # Reformateamos manteniendo el dispositivo y dtype
        hidden_states = hidden_states.squeeze(2).permute(1,0,2).to(dtype) 
