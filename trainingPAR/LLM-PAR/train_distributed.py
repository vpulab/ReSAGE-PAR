...
from contextlib import nullcontext # <--- 1. Añade esta importación arriba del todo


from torch.distributed.fsdp.wrap import size_based_auto_wrap_policy
import functools
from torch.distributed.fsdp.wrap import transformer_auto_wrap_policy
from transformers.models.llama.modeling_llama import LlamaDecoderLayer # O la clase que use el LLM-PAR
from transformers.models.vit.modeling_vit import ViTLayer # Para la parte visual
def setup():
    # Inicializa el grupo de procesos. "nccl" es el backend estándar y más rápido para GPUs Nvidia.
    # Al usar 'torchrun' para lanzar el script, PyTorch lee las IPs y puertos automáticamente de las variables de entorno.
    dist.init_process_group(backend="nccl")
    
    # Esto le dice a FSDP: "Si una capa o bloque tiene más de 100,000 parámetros, envuélvelo como un trozo independiente"
    ...
    # Obtiene qué número de GPU le toca a este proceso en la máquina actual
    local_rank = int(os.environ["LOCAL_RANK"])
    torch.cuda.set_device(local_rank)
    return local_rank, transformer_wrap_policy, mp_policy
...
def main(args):
    ...
    if args.distributed:
        local_rank, transformer_wrap_policy, mp_policy = setup()
    ...
    device = torch.device("cuda") if not args.distributed else torch.device("cuda", local_rank)
    
    if args.distributed:
        sampler_train_distribuido = DistributedSampler(
            dataset=train_set,
            num_replicas=dist.get_world_size(), # Total de GPUs en todo el clúster (ej. 4)
            rank=dist.get_rank(),               # El ID global de esta GPU concreta (0, 1, 2 o 3)
            shuffle=True                        # Mezcla los datos, pero de forma sincronizada entre nodos
        )

        sampler_valid_distribuido = DistributedSampler(
            dataset=valid_set,
            num_replicas=dist.get_world_size(), # Total de GPUs en todo el clúster (ej. 4)
            rank=dist.get_rank(),               # El ID global de esta GPU concreta (0, 1, 2 o 3)
            shuffle=False                       # No mezcla los datos en validación
        )

        train_loader = DataLoader(
            dataset=train_set,
            batch_size=args.batchsize,
            sampler=sampler_train_distribuido,
            shuffle=False,
            num_workers=4,
            pin_memory=True,
            drop_last=True,
        ...
        print(sampler_train_distribuido is None)

        valid_loader = DataLoader(
            dataset=valid_set,
            batch_size=args.batchsize*6,
            sampler=sampler_valid_distribuido,
            shuffle=False,
            num_workers=4,
            pin_memory=True,
        )
    ...
    if args.distributed:
        ...
        modelo = model.to(local_rank)
        modelo_fsdp = FSDP(modelo,
                           auto_wrap_policy=transformer_wrap_policy,
                            mixed_precision=mp_policy, # <--- ¡Mágico para la VRAM!
