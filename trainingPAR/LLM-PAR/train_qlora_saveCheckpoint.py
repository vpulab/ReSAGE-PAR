...
# --- MODIFICACION QLORA 1: Importamos la configuración ---
...
from tools.utils import ReDirectSTD
set_seed(605)
def main(args):
    ...
    # --- MODIFICACION QLORA 2: Creamos la configuración de 4-bits ---
    ...
    # --- MODIFICACION QLORA 3: Pasamos bnb_config al modelo ---
    # Nota: Tendrás que añadir bnb_config=None a la definición de SeqPAR2 en attr2vec_block.py
    ...
    # --- AVISO QLORA --- 
    # Si bnb_config reparte el LLM automáticamente (device_map="auto"), 
    # hacer model.to(device) a todo el modelo puede dar error. 
    # Si te salta un error de "Cannot move 4-bit model to cuda", comenta la línea de abajo:
    ...
    if getattr(args, 'stage1_ckpt_path', None) is not None:
...
def trainer(args, epoch, model, train_loader, valid_loader, optimizer, lr_scheduler, criterion, criterion_llm, path, min_lr, start_epoch):
    ...
    scaler = torch.cuda.amp.GradScaler() # MOVIDO AQUÍ AFUERA DEL BUCLE
    start_epoch = 0
    # Usa args.output_dir si existe, si no, usa el path de logs
    output_dir = getattr(args, 'output_dir', path) 
    resume_path = os.path.join(output_dir, "checkpoint_last.pth")

    if os.path.exists(resume_path):
        print(f"--- Rescatando entrenamiento desde {resume_path} ---")
        checkpoint = torch.load(resume_path, map_location="cpu")
        
        # Cargar pesos del modelo y optimizadores
        missing_keys, unexpected_keys = model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        print(f"--- INFO DE CARGA ---")
        print(f"Llaves ignoradas (Unexpected): {len(unexpected_keys)}")
        print(f"Llaves faltantes (Missing): {len(missing_keys)}")
        
        optimizer['base'].load_state_dict(checkpoint['optimizer_base_state_dict'])
        optimizer['llm'].load_state_dict(checkpoint['optimizer_llm_state_dict'])
        
        # Cargar schedulers para mantener la curva
        lr_scheduler['base'].load_state_dict(checkpoint['scheduler_base_state_dict'])
        lr_scheduler['llm'].load_state_dict(checkpoint['scheduler_llm_state_dict'])
        
        if getattr(args, 'amp', True) and checkpoint.get('scaler_state_dict'):
            scaler.load_state_dict(checkpoint['scaler_state_dict'])
            
        start_epoch = checkpoint['epoch']
        print(f"--- Reiniciando en la época {start_epoch} ---")
    else:
        print("--- Empezando entrenamiento desde cero ---")

    # --- BUCLE PRINCIPAL DE ENTRENAMIENTO ---
    # Usamos start_epoch + 1 para mantener tu misma lógica de índices (ej. Epoch 1)
    for i in range(start_epoch + 1, epoch + 1):
        ...
        model.train()
        print_freq = max(1, int(batch_num / 10))

        for step, (imgs, gt_label, imgname, img_simple_sentences, random_sentences) in enumerate(metric_logger.log_every(train_loader, print_freq, header)):
            imgs, gt_label = imgs.cuda(), gt_label.cuda()
            
            # Step de los Schedulers por batch
            for lrs_name, lrs in lr_scheduler.items():
                lrs.step()
                
            # Control de Learning Rate mínimo
        ...
        # --- GUARDADO ESPECÍFICO POR ÉPOCA (TU CÓDIGO) ---
        ...
        if i % args.eval_freq == 0 and i > 5:
            ...
            with torch.no_grad():
                ...
                for step, (imgs, gt_label, imgname, img_simple_sentences, random_sentences) in enumerate(valid_loader):
                    ...
                    img_names = img_names + list(imgname) # Asegurar compatibilidad de listas
        ...
        # --- GUARDADO DE SEGURIDAD PARA LUSTRE (CHECKPOINT LAST) ---
        checkpoint_last = {
            'epoch': i, 
            'model_state_dict': model.state_dict(),
            'optimizer_base_state_dict': optimizer['base'].state_dict(),
            'optimizer_llm_state_dict': optimizer['llm'].state_dict(),
            'scheduler_base_state_dict': lr_scheduler['base'].state_dict(),
            'scheduler_llm_state_dict': lr_scheduler['llm'].state_dict(),
            'scaler_state_dict': scaler.state_dict() if getattr(args, 'amp', True) else None,
        }
        torch.save(checkpoint_last, os.path.join(output_dir, "checkpoint_last.pth")) 
