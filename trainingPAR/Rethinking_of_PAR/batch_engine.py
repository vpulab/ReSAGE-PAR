...
# Conditional import for MixUp and CutMix
try:
    from torchvision.transforms import v2
    DATAAUG_V2_AVAILABLE = True
except ImportError:
    DATAAUG_V2_AVAILABLE = False
...
def batch_trainer(cfg, args, epoch, model, model_ema, train_loader, criterion, optimizer, loss_w=[1, ], scheduler=None):
    ...
    # Initialize MixUp and CutMix if enabled
    mixup = None
    cutmix = None
    if cfg.TRAIN.DATAAUG.ENABLE and DATAAUG_V2_AVAILABLE:
        if cfg.TRAIN.DATAAUG.TYPE == 'mixup':
            mixup = v2.MixUp(num_classes=cfg.TRAIN.DATAAUG.NUMCLASSES)
        elif cfg.TRAIN.DATAAUG.TYPE == 'cutmix':
            cutmix = v2.CutMix(num_classes=cfg.TRAIN.DATAAUG.NUMCLASSES)
    ...
    for step, (imgs, gt_label, imgname) in enumerate(train_loader):
        ...
        # Apply MixUp or CutMix if initialized
        if mixup is not None:
            imgs, gt_label = mixup(imgs, gt_label)
        elif cutmix is not None:
            imgs, gt_label = cutmix(imgs, gt_label)
        ...
        if (step + 1) % log_interval == 0 or (step + 1) % len(train_loader) == 0:
            if args.local_rank == 0:
                ...
                mlflow.log_metric("training_loss", f"{loss_meter.avg:3f}", step=(step // batch_num))
                #mlflow.log_metric("training_loss", f"{loss_meter.avg:3f}", step=(step // batch_num))
            # break
...
def valid_trainer(cfg, args, epoch, model, valid_loader, criterion, loss_w=[1, ]):
    ...
    mlflow.log_metric("testing_loss", f"{valid_loss:3f}", step=mlflowStepVal)
    mlflowStepVal +=1
