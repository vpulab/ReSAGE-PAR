...
def get_pedestrian_metrics(gt_label, preds_probs, threshold=0.5, index=None, cfg=None):
    ...
    instance_acc_label = intersect_pos / (union_pos + eps)
    ...
    result.instance_acc_label = instance_acc_label
    result.instance_prec_label = instance_prec_label
    result.instance_recall_label = instance_recall_label
    result.instance_f1_label = instance_f1_label


    instance_acc = np.mean(instance_acc_label)
    instance_prec = np.mean(instance_prec_label)
    instance_recall = np.mean(instance_recall_label)
