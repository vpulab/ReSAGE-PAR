...
set_seed(605)
device = "cuda"
clip_model, ViT_preprocess = clip.load("ViT-L/14", device=device,download_root='data/models') #选择image特征提取器

def main(args):
    start_time=time_str()
    print(f'start_time is {start_time}')
    
    if args.syn_use and args.synthetic_path:
        syn_name = os.path.basename(os.path.normpath(args.synthetic_path))
        log_dir = os.path.join('logs', args.dataset, f'syn_{syn_name}')
    else:
